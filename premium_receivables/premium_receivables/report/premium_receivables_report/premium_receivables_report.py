...
def get_columns(filters):
    ...
    return [
        {"label": _("Date of Payment Entry"), "fieldname": "posting_date", "fieldtype": "Date", "width": 130},
        ...
        {"label": _("Client Name"), "fieldname": "party", "fieldtype": "Data", "width": 180},
        {"label": _("Receivable Account"), "fieldname": "party_account", "fieldtype": "Link", "options": "Account", "width": 180},
        {"label": _("Client Contact"), "fieldname": "customer_primary_contact", "fieldtype": "Data", "width": 130},
        ...
        {"label": _("Invoice Number"), "fieldname": "voucher_no", "fieldtype": "Data", "width": 200},
        {"label": _("Due Date"), "fieldname": "due_date", "fieldtype": "Date", "width": 90},
        {"label": _("Premium Due"), "fieldname": "invoiced", "fieldtype": "Currency", "options": "currency", "width": 130},
        {"label": _("Premium Paid"), "fieldname": "paid", "fieldtype": "Currency", "options": "currency", "width": 130},
        ...
        {"label": _("Client Group"), "fieldname": "customer_group", "fieldtype": "Link", "options": "Customer Group", "width": 130},
    ...
    for key, row in voucher_balance.items():
        ...
        if against_type == "Sales Invoice":
            payment = frappe.db.sql("""
                SELECT pe.name as receipt_number,
        ...
        # si details
        si_details = frappe._dict()
        if against_type == "Sales Invoice":
            si = frappe.db.get_value("Sales Invoice", against_no,
                ["territory", "customer_group", "contact_person", "remarks"], as_dict=True)
            if si:
                si_details = si

        # sales person
        sp = frappe.db.sql("""
            SELECT GROUP_CONCAT(sales_person SEPARATOR ", ")
            FROM `tabSales Team`
            WHERE parent = %(voucher)s AND parenttype = "Sales Invoice"
        """, {"voucher": against_no})
        sales_person = sp[0][0] if sp and sp[0][0] else ""
        ...
        # arrears in months — ceiling, only when overdue
        ...
        data.append({
            ...
            "customer_primary_contact": si_details.get("contact_person", ""),
            "cost_center": row.cost_center,
            "voucher_type": against_type,
            ...
            "territory": si_details.get("territory", ""),
            "customer_group": si_details.get("customer_group", ""),
            "sales_person": sales_person,
            "remarks": si_details.get("remarks", ""),
        })
...
def get_chart_data(data, filters):
    ...
    rows = []
    for row in data:
        # only include rows with positive outstanding for chart
